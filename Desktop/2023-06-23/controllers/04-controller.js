...
const deletePersonaje = (personajeID) => {
  // Tu código aquí
  const filtrados = utils.personajes.filter(personaje => personaje.id !== personajeID)
  if(filtrados.length === utils.personajes.length) throw new Error(`No existe el personaje con id: ${personajeID}`)
  utils.personajes = filtrados
